...
const SOCIALS: Social[] = [
  ...
  {
    label: "Instagram",
    href: "https://instagram.com/fuad24fps",
    brand: "#E4405F",
  ...
  {
    label: "Facebook",
    href: "https://facebook.com/fuad24fps",
    brand: "#1877F2",
  ...
  {
    label: "LinkedIn",
    href: "https://linkedin.com/in/fuad24fps",
    brand: "#0A66C2",
...
/* Same easing curve the dialogs use, so the whole site decelerates alike. */
const EASE = [0.22, 1, 0.36, 1] as const;


const PHOTO = {
  hidden: { opacity: 0, x: -28, scale: 0.96, rotate: -3 },
  show: {
    opacity: 1,
    x: 0,
    scale: 1,
    rotate: 0,
    transition: { type: "spring" as const, stiffness: 190, damping: 26, mass: 1 },
  },
};

const COPY = {
  hidden: {},
  show: { transition: { delayChildren: 0.12, staggerChildren: 0.085 } },
};

const LINE = {
  hidden: { opacity: 0, y: 18 },
  show: { opacity: 1, y: 0, transition: { duration: 0.80, ease: EASE } },
};

const VIEWPORT = { once: true, amount: .40 } as const;
...
export default function About() {
  ...
  return (
    ...
    <section id="about" className="mx-auto max-w-285 overflow-x-clip px-5 py-16 sm:px-8 md:py-20">
      <div className="relative rounded-[28px] border-[1.5px] border-white/90 bg-white/70 p-6 shadow-[0_20px_60px_-20px_rgba(31,36,48,.22)] backdrop-blur-lg sm:p-9 md:p-12">
        <div className="flex flex-col items-center gap-9 md:flex-row md:items-center md:gap-14">
          ...
          >
            <motion.h3 variants={LINE} className="flex items-center gap-2 font-mono text-[11px] font-bold uppercase tracking-[.2em] text-emerald-700">
              <span className="hidden h-px w-8 bg-emerald-400 md:inline-block" />
              Creative profile
            </motion.h3>

            <motion.h2 variants={LINE} className="mt-3 text-[clamp(3.25rem,14vw,5rem)] font-black leading-[0.9] tracking-[-0.03em] text-ink">
              {profile.name}.
            </motion.h2>

            <motion.p variants={LINE} className="relative mt-4 overflow-hidden rounded-tl-2xl rounded-br-2xl bg-ink px-5 py-2.5 text-base font-bold text-white shadow-md sm:text-lg md:text-xl">
              {profile.role}
            </motion.p>

            {/* rule sits on top when centred, on the left once the text is left-aligned */}
            <motion.p variants={LINE} className="mt-6 max-w-xl border-t-2 border-emerald-200 pt-4 text-base font-medium leading-relaxed text-muted md:border-l-4 md:border-t-0 md:pl-6 md:pt-0 md:text-lg">
              {profile.description}
            </motion.p>

            {/* the row animates, not the button inside it — motion sets an inline
                transform and would override the button's hover:scale-105 */}
            <motion.div
              variants={LINE}
